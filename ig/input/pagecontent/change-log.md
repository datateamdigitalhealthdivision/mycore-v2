...
Version 2.1 focuses the guide on three use cases — **ADT and discharge summary, laboratory reports, radiology reports** — and publishes the national terminology those use cases bind to.
...
Observation is now two profiles. Anything that differs only by the value in `code` — fasting against random glucose, systolic against diastolic blood pressure — is a value set entry rather than a separate profile, and blood pressure keeps the standard two-component shape that consuming systems expect.
...
- **Identifier namespaces** consolidated to a single scheme, `https://id.kkmhub.moh.gov.my/{domain}/{identifier}`.
